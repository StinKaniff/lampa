(function () {
  ...
  const PLUGIN_GUARD_KEY = '__APPLE_NATIVE__';
  ...
  const APPLE_NATIVE_KEYS = {
    STYLE_ID: 'apple-native-style',
    BODY_CLASS: 'apple-native',
    CLOCK_ID: 'apple-native-clock',
    TMDB_KEY: '4ef0d7355d9ffb5151e987764708ce96',
    ENABLE_KEY: 'apple_native_topnav_enabled',
    TOPNAV_ITEMS_KEY: 'apple_native_topnav_items',
    LOGO_LANG_KEY: 'apple_native_logo_lang',
    FONT_SIZE_KEY: 'apple_native_font_size',
    UI_LANG_KEY: 'apple_native_ui_lang',
    BACKDROP_KEY: 'apple_native_backdrop',
    BADGE_KEY: 'apple_native_badge',
    RATING_KEY: 'apple_native_rating',
    RATING_STYLE_KEY: 'apple_native_rating_style',
    CATEGORY_SIZE_KEY: 'apple_native_category_size',
    CARD_SIZE_KEY: 'apple_native_card_size',
    CLOCK_SECONDS_KEY: 'apple_native_clock_seconds',
    CONTROL_PANEL_KEY: 'apple_native_control_panel',
    PERF_MODE_KEY: 'apple_native_perf_mode',
    SETTINGS_COMPONENT: 'apple_native',
    TOPNAV_SETTINGS_COMPONENT: 'apple_native_topnav',
    ...
    LOGO_SIZE_KEY: 'apple_native_logo_size',
    LOGO_SIZE_ATTR: 'data-agnative-logo-size',
    CACHE_SIZE_KEY: 'apple_native_cache_size',
    POSTER_QUALITY_KEY: 'apple_native_poster_quality',
    PERF_ATTR: 'data-agnative-perf',
    FLEX_GAP_ATTR: 'data-agnative-flex-gap',
    OVERLAY_ALIGN_KEY: 'apple_native_overlay_align',
    OVERLAY_ALIGN_ATTR: 'data-agnative-overlay-align',
    CARD_IMAGE_MODE_KEY: 'apple_native_card_image_mode',
    CARD_IMAGE_MODE_ATTR: 'data-agnative-card-image-mode',
    LOGO_TITLE_KEY: 'apple_native_logo_title_fallback'
  };
  ...
  const en = {
    ...
    set_reset_done: 'Apple Native settings reset',
  ...
  const uk = {
    ...
    set_reset_done: 'Налаштування Apple Native скинуто',
  ...
  function registerI18nToLampa() {
    ...
    if (window.__APPLE_NATIVE_I18N_REGISTERED__) return;
    ...
    window.__APPLE_NATIVE_I18N_REGISTERED__ = true;
  }

  var DB_NAME = 'apple-native-cache';
  var DB_VERSION = 1;
  var STORE_META = 'meta';
  var STORE_IMG = 'img';
  ...
  (function () {
    ...
    function detectLampaLang() {
      try {
        ...
        if (l.indexOf('ru') === 0) return 'uk';
        if (l.indexOf('be') === 0) return 'uk';
        return 'uk';
      } catch (e) { return 'uk'; }
    }
    ...
    function getLogoLang() {
      try {
        if (!window.Lampa || !Lampa.Storage) return 'uk';
    ...
    function removePluginUi() {
      try {
        ...
        styleSignature = '';
        var shell = document.querySelector('.apple-native-topnav-shell');
        if (shell) shell.remove();
        var dock = document.querySelector('.apple-native-topnav-rightdock');
        if (dock) dock.remove();
        var clock = document.getElementById(CLOCK_ID);
        if (clock) clock.remove();
        var panel = document.querySelector('.apple-native-control-panel');
        if (panel) panel.remove();
        var leftdock = document.querySelector('.apple-native-leftdock');
        ...
        if (headEl && headEl.__appleNativeWheelBound) {
          headEl.removeEventListener('wheel', forwardWheelBelowTopnav, { passive: false });
          headEl.__appleNativeWheelBound = false;
        }
    ...
    function registerSettings() {
      try {
        ...
        Lampa.SettingsApi.addComponent({
          ...
          icon: '<span style="font-size:1em;line-height:1;display:flex;align-items:center;justify-content:center;">􀺰</span>',
          name: 'Apple Native'
        });

        Lampa.SettingsApi.addParam({
          component: SETTINGS_COMPONENT,
          param: { name: 'apple_native_about_info', type: 'static' },
          field: {
            name: 'Apple Native',
        ...
        Lampa.SettingsApi.addParam({
          component: SETTINGS_COMPONENT,
          param: { name: 'apple_native_open_topnav_settings', type: 'button' },
          field: {
        ...
        Lampa.SettingsApi.addParam({
          component: SETTINGS_COMPONENT,
          param: { name: 'apple_native_reset_button', type: 'button' },
          field: {
        ...
        getAvailableTopnavItems().forEach(function (item) {
          Lampa.SettingsApi.addParam({
            component: TOPNAV_SETTINGS_COMPONENT,
            param: {
              name: 'apple_native_topnav_item_' + item.action,
              type: 'select',
    ...
    function getGenreNames(item) {
      ...
      var map = GENRE_MAP_LOCALIZED[getUiLang()] || GENRE_MAP_LOCALIZED.en;
    ...
    function injectStyle() {
      ...
      style.id = STYLE_ID;
      var text = [
        '@font-face { font-family: "SF Symbols"; src: url("https://cdn.jsdelivr.net/gh/mykhailo-pashynskyi/lampa@main/ui-apple/SFSymbols.otf") format("opentype"); font-weight: normal; font-style: normal; font-display: block; }',
        ...
        '  margin: 0 !important;',
        '  font-size: calc(.78em * var(--apple-native-scale, 1)) !important;',
        '  font-weight: 700 !important;',
        ...
        '  display: none !important;',
        '}',
        'body.' + BODY_CLASS + ' .apple-native-topnav-shell { position:absolute; left:50%; top:.46em; transform:translateX(-50%); z-index:20; width:max-content; max-width:calc(100vw - 24em); height:2.6em; display:inline-flex; align-items:center; box-sizing:border-box; }',
        'body.' + BODY_CLASS + ' .apple-native-topnav-shell__inner { height:2.6em; box-sizing:border-box; display:inline-flex; align-items:center; gap:.18em; padding:.21em .32em; border-radius:999px; background:rgba(22,24,30,.28); border:1px solid rgba(255,255,255,.10); box-shadow:inset 0 1px 0 rgba(255,255,255,.10), 0 8px 18px rgba(0,0,0,.12); backdrop-filter:blur(18px) saturate(140%); -webkit-backdrop-filter:blur(18px) saturate(140%); }',
        'body.' + BODY_CLASS + ' .apple-native-topnav-shell__items { display:flex; align-items:center; justify-content:center; gap:.08em; }',
        'body.' + BODY_CLASS + ' .apple-native-topnav-shell__right { display:flex; align-items:center; gap:.08em; }',
        'body.' + BODY_CLASS + ' .apple-native-topnav-shell__item.selector { appearance:none; -webkit-appearance:none; border:0; background:none; color:rgba(255,255,255,.92); height:2.16em; display:inline-flex; align-items:center; justify-content:center; text-align:center; padding:0 .96em; border-radius:999px; font-size:.83em; font-weight:700; line-height:1; white-space:nowrap; transition:background .2s ease, transform .2s ease, box-shadow .2s ease; }',
        'body.' + BODY_CLASS + ' .apple-native-topnav-shell__item--icon.selector { width:2.16em; min-width:2.16em; padding:0; }',
        'body.' + BODY_CLASS + ' .apple-native-topnav-shell__item--icon svg { width:1em; height:1em; }',
        'body.' + BODY_CLASS + ' .apple-native-topnav-right__profile.selector { width:2.16em; min-width:2.16em; padding:0; overflow:hidden; }',
        'body.' + BODY_CLASS + ' .apple-native-topnav-right__profile-img { width:1.56em; height:1.56em; border-radius:999px; object-fit:cover; display:block; }',
        'body.' + BODY_CLASS + ' .apple-native-topnav-shell__item.is-active, body.' + BODY_CLASS + ' .apple-native-topnav-shell__item.hover, body.' + BODY_CLASS + ' .apple-native-topnav-shell__item.focus { background:rgba(255,255,255,.14); box-shadow:inset 0 1px 0 rgba(255,255,255,.10); }',
        'body.' + BODY_CLASS + ' .apple-native-topnav-rightdock { position:absolute; right:1.15em; top:.46em; z-index:20; height:2.6em; box-sizing:border-box; display:inline-flex; align-items:center; gap:.08em; padding:.21em .32em; border-radius:999px; background:rgba(22,24,30,.28); border:1px solid rgba(255,255,255,.10); box-shadow:inset 0 1px 0 rgba(255,255,255,.10), 0 8px 18px rgba(0,0,0,.12); backdrop-filter:blur(18px) saturate(140%); -webkit-backdrop-filter:blur(18px) saturate(140%); }',
        'body.' + BODY_CLASS + ' .apple-native-topnav-rightdock .apple-native-topnav-clock { position:static !important; right:auto !important; top:auto !important; z-index:auto !important; height:2.16em !important; min-width:4.2em !important; padding:0 .95em !important; background:transparent !important; border:0 !important; box-shadow:none !important; backdrop-filter:none !important; -webkit-backdrop-filter:none !important; transition:background .2s ease, transform .2s ease, box-shadow .2s ease; }',
        'body.' + BODY_CLASS + ' .apple-native-topnav-rightdock .apple-native-topnav-clock.hover, body.' + BODY_CLASS + ' .apple-native-topnav-rightdock .apple-native-topnav-clock.focus, body.' + BODY_CLASS + ' .apple-native-topnav-rightdock .apple-native-topnav-right__profile.hover, body.' + BODY_CLASS + ' .apple-native-topnav-rightdock .apple-native-topnav-right__profile.focus { background:rgba(255,255,255,.14) !important; box-shadow:inset 0 1px 0 rgba(255,255,255,.10) !important; transform:translateY(-.02em); }',
        'body.' + BODY_CLASS + ' .apple-native-topnav-clock { position:absolute; right:1.15em; top:.46em; z-index:20; display:inline-flex; align-items:center; justify-content:center; min-width:4.2em; height:2.6em; padding:0 .95em; border-radius:999px; background:rgba(22,24,30,.26); border:1px solid rgba(255,255,255,.10); box-shadow:inset 0 1px 0 rgba(255,255,255,.10), 0 8px 18px rgba(0,0,0,.12); backdrop-filter:blur(18px) saturate(140%); -webkit-backdrop-filter:blur(18px) saturate(140%); color:rgba(255,255,255,.95); font-size:.92em; font-weight:700; letter-spacing:.01em; }',
        'body.' + BODY_CLASS + ' .apple-native-topnav-clock.selector { cursor:pointer; }',
        'body.' + BODY_CLASS + ' .apple-native-control-panel { position:absolute; right:1.15em; top:3.7em; z-index:26; width:18.8em; padding:.72em; border-radius:1.18em; background:rgba(40,48,62,.76); border:1px solid rgba(255,255,255,.13); box-shadow:0 18px 48px rgba(0,0,0,.28), inset 0 1px 0 rgba(255,255,255,.09); backdrop-filter:blur(22px) saturate(136%); -webkit-backdrop-filter:blur(22px) saturate(136%); opacity:0; transform:translateY(-.35em) scale(.98); pointer-events:none; transition:opacity .2s ease, transform .2s ease; }',
        'body.' + BODY_CLASS + ' .apple-native-control-panel.is-open { opacity:1; transform:translateY(0) scale(1); pointer-events:auto; }',
        'body.' + BODY_CLASS + ' .apple-native-control-panel__title { font-size:1.28em; font-weight:600; color:rgba(255,255,255,.94); padding:.18em .15em .52em; }',
        'body.' + BODY_CLASS + ' .apple-native-control-panel__grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:.48em; }',
        'body.' + BODY_CLASS + ' .apple-native-control-panel__tile.selector { min-height:5.2em; border-radius:.95em; background:rgba(16,20,28,.82); border:1px solid rgba(255,255,255,.10); display:flex; flex-direction:column; align-items:flex-start; justify-content:center; gap:.34em; padding:.65em .72em; color:rgba(255,255,255,.95); transition:background .2s ease, box-shadow .2s ease, transform .2s ease; }',
        'body.' + BODY_CLASS + ' .apple-native-control-panel__tile.selector .apple-native-control-panel__icon { width:1.3em; height:1.3em; display:inline-flex; align-items:center; justify-content:center; color:rgba(214,230,255,.97); }',
        'body.' + BODY_CLASS + ' .apple-native-control-panel__tile.selector .apple-native-control-panel__icon svg { width:1.3em; height:1.3em; }',
        'body.' + BODY_CLASS + ' .apple-native-control-panel__tile.selector .apple-native-control-panel__label { font-size:.95em; font-weight:700; line-height:1.15; text-align:left; }',
        'body.' + BODY_CLASS + ' .apple-native-control-panel__tile.selector.hover, body.' + BODY_CLASS + ' .apple-native-control-panel__tile.selector.focus { background:rgba(255,255,255,.18); box-shadow:inset 0 1px 0 rgba(255,255,255,.18), 0 0 0 1px rgba(255,255,255,.12); transform:translateY(-.02em); }',
        ...
        'body.' + BODY_CLASS + ' .items-line--type-default .items-line__head { margin-bottom:.58em !important; min-height:auto !important; padding-top:1em !important; padding-bottom:.5em !important; padding-left:1.5em !important; padding-right:1em !important; font-size:1em !important; }',
        ...
        'body.' + BODY_CLASS + ' .scroll__body.mapping--line { display:flex !important; gap:1.5em !important; padding-left:0 !important; padding-right:1.15em !important; }',
        ...
        'body.' + BODY_CLASS + ' [data-nfx-continue-row] .card { width:22em !important; }',
        'body.' + BODY_CLASS + ' [data-nfx-continue-row] .card .card__view { padding-bottom:56.25% !important; }',
        'body.' + BODY_CLASS + ' [data-nfx-continue-row] .card .card__img { object-position:center center !important; }',
        'body.' + BODY_CLASS + ' .nfx-continue-episode { color:rgba(255,255,255,.92) !important; font-size:.74em !important; font-weight:600 !important; letter-spacing:.02em !important; text-shadow:0 1px 6px rgba(0,0,0,.5) !important; margin-top:0 !important; line-height:1.3 !important; }',
        ...
        'body.' + BODY_CLASS + ' .card.focus .card__view { transform: translateY(-.08em) scale(1.12) !important; filter: saturate(1.06) brightness(1.02) !important; box-shadow: inset 0 1px 0 rgba(255,255,255,.22), 0 18px 42px rgba(0,0,0,.26), 0 8px 20px rgba(0,0,0,.14) !important; }',
        ...
        'body.' + BODY_CLASS + ' .card.focus .card__view::after { content:"" !important; display:block !important; position:absolute !important; inset:0 !important; border-radius:1.35em !important; background:rgba(6,8,14,.2) !important; pointer-events:none !important; z-index:3 !important; }',
        ...
        'body.' + BODY_CLASS + ' .card-episode .full-episode__num { position:absolute !important; top:.72em !important; right:.82em !important; left:auto !important; bottom:auto !important; z-index:4 !important; display:inline-flex !important; align-items:center !important; justify-content:center !important; min-width:1.9em !important; font-size: calc(.72em * var(--apple-native-scale, 1)) !important; font-weight:800 !important; color:#fff !important; letter-spacing:.04em !important; line-height:1 !important; margin:0 !important; padding:.34em .66em !important; border-radius:.85em !important; background:rgba(12,14,20,.68) !important; border:1px solid rgba(255,255,255,.14) !important; backdrop-filter: blur(10px) saturate(140%) !important; -webkit-backdrop-filter: blur(10px) saturate(140%) !important; box-shadow: 0 4px 10px rgba(0,0,0,.24) !important; text-shadow:none !important; }',
        ...
        'body.' + BODY_CLASS + ' .card-episode .nfx-episode-title { font-size: calc(.95em * var(--apple-native-scale, 1)) !important; font-weight:800 !important; color:#fff !important; line-height:1.14 !important; max-width:100% !important; overflow:hidden !important; text-overflow:ellipsis !important; white-space:nowrap !important; margin:0 0 .3em !important; padding:0 !important; letter-spacing:.005em !important; }',
        'body.' + BODY_CLASS + ' .card-episode .full-episode__name { font-size: calc(.82em * var(--apple-native-scale, 1)) !important; font-weight:700 !important; color:#fff !important; line-height:1.18 !important; max-width:100% !important; overflow:hidden !important; text-overflow:ellipsis !important; white-space:nowrap !important; margin:0 !important; padding:0 !important; }',
        'body.' + BODY_CLASS + ' .card-episode .full-episode__date { font-size: calc(.68em * var(--apple-native-scale, 1)) !important; font-weight:500 !important; color:rgba(255,255,255,.78) !important; line-height:1.2 !important; margin-top:.18em !important; padding:0 !important; letter-spacing:.01em !important; gap: .5em;}',
        ...
        'body.' + BODY_CLASS + ' .card-episode.focus .full-episode__img { filter: saturate(1.06) brightness(1.02) !important; box-shadow: inset 0 1px 0 rgba(255,255,255,.22), 0 18px 42px rgba(0,0,0,.26), 0 8px 20px rgba(0,0,0,.14) !important; }',
        ...
        'body.' + BODY_CLASS + ' .nfx-card-overlay__logo, body.' + BODY_CLASS + ' img.nfx-card-overlay__logo { display:block !important; opacity:1 !important; visibility:visible !important; max-height:2.55em !important; max-width:82% !important; margin-bottom:0 !important; border-radius:0 !important; clip-path:none !important; -webkit-clip-path:none !important; mask-image:none !important; -webkit-mask-image:none !important; overflow:visible !important; }',
        ...
        'body.' + BODY_CLASS + ' .nfx-card-logo { position:static !important; display:block !important; opacity:1 !important; visibility:visible !important; padding:0 !important; border-radius:0 !important; background:none !important; border:none !important; backdrop-filter:none !important; -webkit-backdrop-filter:none !important; color:rgba(255,255,255,.88) !important; font-size:.74em !important; font-weight:500 !important; letter-spacing:.01em !important; text-shadow:0 1px 8px rgba(0,0,0,.45) !important; margin-top:0 !important; line-height:1.28 !important; white-space:normal !important; pointer-events:none; }',
        'body.' + BODY_CLASS + ' { --apple-native-scale: 1; --apple-native-category-scale: 1; font-family: "SF Pro Display", "SF Pro Text", -apple-system, BlinkMacSystemFont, "Helvetica Neue", Arial, sans-serif !important; }',
        'body.' + BODY_CLASS + '[' + FONT_SIZE_ATTR + '="xs"] { --apple-native-scale: .85; }',
        'body.' + BODY_CLASS + '[' + FONT_SIZE_ATTR + '="sm"] { --apple-native-scale: .92; }',
        'body.' + BODY_CLASS + '[' + FONT_SIZE_ATTR + '="md"] { --apple-native-scale: 1; }',
        'body.' + BODY_CLASS + '[' + FONT_SIZE_ATTR + '="lg"] { --apple-native-scale: 1.12; }',
        'body.' + BODY_CLASS + '[' + FONT_SIZE_ATTR + '="xl"] { --apple-native-scale: 1.24; }',
        'body.' + BODY_CLASS + '[' + CATEGORY_SIZE_ATTR + '="xs"] { --apple-native-category-scale: .78; }',
        'body.' + BODY_CLASS + '[' + CATEGORY_SIZE_ATTR + '="sm"] { --apple-native-category-scale: .9; }',
        'body.' + BODY_CLASS + '[' + CATEGORY_SIZE_ATTR + '="md"] { --apple-native-category-scale: 1; }',
        'body.' + BODY_CLASS + '[' + CATEGORY_SIZE_ATTR + '="lg"] { --apple-native-category-scale: 1.18; }',
        'body.' + BODY_CLASS + '[' + CATEGORY_SIZE_ATTR + '="xl"] { --apple-native-category-scale: 1.4; }',
        'body.' + BODY_CLASS + ' .apple-native-topnav-shell__item.selector { font-size: calc(.85em * var(--apple-native-scale, 1)) !important; }',
        'body.' + BODY_CLASS + ' .apple-native-topnav-clock { font-size: calc(.88em * var(--apple-native-scale, 1)) !important; }',
        'body.' + BODY_CLASS + ' .items-line__title { font-size: calc(1em * var(--apple-native-scale, 1) * var(--apple-native-category-scale, 1)) !important; }',
        'body.' + BODY_CLASS + ' .items-line__more.selector { font-size: calc(.72em * var(--apple-native-scale, 1) * var(--apple-native-category-scale, 1)) !important; }',
        'body.' + BODY_CLASS + ' .nfx-card-overlay__title { font-size: calc(1em * var(--apple-native-scale, 1)) !important; }',
        'body.' + BODY_CLASS + ' .nfx-card-overlay__meta { font-size: calc(.72em * var(--apple-native-scale, 1)) !important; }',
        'body.' + BODY_CLASS + ' .nfx-card-logo { font-size: calc(.72em * var(--apple-native-scale, 1)) !important; }',
        'body.' + BODY_CLASS + ' .nfx-card-rating { position:absolute; top:.7em; right:.82em; z-index:4; display:inline-flex; align-items:center; justify-content:center; padding:.32em .66em; border-radius:.85em; background:rgba(12,14,20,.68); border:1px solid rgba(255,255,255,.14); color:rgba(255,255,255,.96); font-size: calc(.72em * var(--apple-native-scale, 1)); font-weight:800; letter-spacing:.02em; backdrop-filter: blur(10px) saturate(140%); -webkit-backdrop-filter: blur(10px) saturate(140%); pointer-events:none; box-shadow: 0 4px 10px rgba(0,0,0,.24); }',
        ...
        'body.' + BODY_CLASS + '[' + BACKDROP_ATTR + '="off"] [data-nfx-continue-row] .card .card__view { padding-bottom:56.25% !important; }',
        'body.' + BODY_CLASS + '[' + BACKDROP_ATTR + '="off"] [data-nfx-continue-row] .card { width:22em !important; }',
        ...
        'body.' + BODY_CLASS + '[' + BACKDROP_ATTR + '="off"] .nfx-card-overlay { display:flex !important; height:35% !important; top:auto !important; background:transparent !important; backdrop-filter:none !important; -webkit-backdrop-filter:none !important; padding:1em !important; gap:.5em !important; justify-content:center !important; align-items:center !important; border-radius:0 0 1.35em 1.35em !important; overflow:visible !important; }',
        'body.' + BODY_CLASS + '[' + BACKDROP_ATTR + '="off"] .nfx-card-overlay::before { content:"" !important; position:absolute !important; inset:0 !important; border-radius:0 0 1.35em 1.35em !important; backdrop-filter:blur(22px) saturate(180%) !important; -webkit-backdrop-filter:blur(22px) saturate(180%) !important; -webkit-mask-image:linear-gradient(to bottom, transparent 0%, black 100%) !important; mask-image:linear-gradient(to bottom, transparent 0%, black 100%) !important; z-index:0 !important; pointer-events:none !important; }',
        'body.' + BODY_CLASS + '[' + BACKDROP_ATTR + '="off"] .nfx-card-overlay > * { position:relative !important; z-index:1 !important; }',
        'body.' + BODY_CLASS + '[' + BACKDROP_ATTR + '="off"] .nfx-card-rating { display:inline-flex !important; }',
        'body.' + BODY_CLASS + '[' + BACKDROP_ATTR + '="off"] .nfx-card-logo { display:block !important; }',
        ...
        'body.' + BODY_CLASS + '[' + BACKDROP_ATTR + '="off"] .nfx-card-overlay__logo { max-height:2.4em !important; max-width:82% !important; margin:0 auto !important; filter:drop-shadow(0 1px 10px rgba(0,0,0,.7)) !important; }',
        ...
        'body.' + BODY_CLASS + '[' + BACKDROP_ATTR + '="off"] .nfx-card-overlay__title { font-size: calc(.95em * var(--apple-native-scale, 1)) !important; font-weight:800 !important; line-height:1.16 !important; white-space:normal !important; display:-webkit-box !important; -webkit-line-clamp:2 !important; -webkit-box-orient:vertical !important; overflow:hidden !important; }',
        'body.' + BODY_CLASS + '[' + BACKDROP_ATTR + '="off"] .nfx-card-overlay__meta { font-size: calc(.68em * var(--apple-native-scale, 1)) !important; margin-top:.18em !important; opacity:.85 !important; white-space:normal !important; }',
        ...
        '  font-size: calc(1em * var(--apple-native-scale, 1) * var(--apple-native-category-scale, 1)) !important;',
        ...
        'body.' + BODY_CLASS + '[' + FLEX_GAP_ATTR + '="no"] .apple-native-topnav-shell__inner { gap: 0 !important; }',
        'body.' + BODY_CLASS + '[' + FLEX_GAP_ATTR + '="no"] .apple-native-topnav-shell__inner > * + * { margin-left: .18em !important; }',
        'body.' + BODY_CLASS + '[' + FLEX_GAP_ATTR + '="no"] .apple-native-topnav-shell__items { gap: 0 !important; }',
        'body.' + BODY_CLASS + '[' + FLEX_GAP_ATTR + '="no"] .apple-native-topnav-shell__items > * + * { margin-left: .08em !important; }',
        'body.' + BODY_CLASS + '[' + FLEX_GAP_ATTR + '="no"] .apple-native-topnav-shell__right { gap: 0 !important; }',
        'body.' + BODY_CLASS + '[' + FLEX_GAP_ATTR + '="no"] .apple-native-topnav-shell__right > * + * { margin-left: .08em !important; }',
        'body.' + BODY_CLASS + '[' + FLEX_GAP_ATTR + '="no"] .apple-native-topnav-rightdock { gap: 0 !important; }',
        'body.' + BODY_CLASS + '[' + FLEX_GAP_ATTR + '="no"] .apple-native-topnav-rightdock > * + * { margin-left: .08em !important; }',
        ...
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .apple-native-topnav-shell__inner,',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .apple-native-topnav-rightdock,',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .apple-native-topnav-clock,',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .apple-native-control-panel,',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .apple-native-control-panel__tile,',
        ...
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .apple-native-topnav-shell__inner,',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .apple-native-topnav-rightdock,',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .apple-native-topnav-clock { background: rgba(22,24,30,.86) !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .apple-native-control-panel { background: rgba(40,48,62,.95) !important; }',
        ...
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .apple-native-topnav-shell__item { transition-duration: .12s !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .card.focus .card__view { filter: none !important; box-shadow: 0 6px 14px rgba(0,0,0,.22) !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .card.hover .card__view { filter: none !important; box-shadow: 0 6px 14px rgba(0,0,0,.18) !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .card-episode.focus .full-episode__img { filter: none !important; box-shadow: 0 6px 14px rgba(0,0,0,.22) !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .card-episode.hover .full-episode__img { filter: none !important; box-shadow: 0 6px 14px rgba(0,0,0,.18) !important; }',
        ...
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .apple-native-topnav-shell__item,',
        ...
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .card.focus .card__view { transform: none !important; filter: none !important; box-shadow: 0 2px 6px rgba(0,0,0,.4) !important; box-sizing: border-box !important; border-color: rgba(255,255,255,.18) !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .card.hover .card__view { transform: none !important; filter: none !important; box-shadow: 0 2px 6px rgba(0,0,0,.4) !important; box-sizing: border-box !important; border-color: rgba(255,255,255,.10) !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .card-episode.focus .full-episode__img { filter: none !important; box-shadow: 0 2px 6px rgba(0,0,0,.4) !important; box-sizing: border-box !important; border-color: rgba(255,255,255,.18) !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .card-episode.hover .full-episode__img { filter: none !important; box-shadow: 0 2px 6px rgba(0,0,0,.4) !important; box-sizing: border-box !important; border-color: rgba(255,255,255,.10) !important; }',
        ...
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .apple-native-topnav-shell__inner,',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .apple-native-topnav-rightdock,',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .apple-native-topnav-clock { background: rgba(22,24,30,.96) !important; box-shadow: none !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .apple-native-control-panel { background: rgba(28,30,34,.98) !important; box-shadow: 0 4px 10px rgba(0,0,0,.4) !important; }',
        ...
        'body.' + BODY_CLASS + ' .apple-native-leftdock {',
        ...
        '  font-size: calc(clamp(0.92rem, 0.5rem + 1.4vmin, 1.7rem) * var(--apple-native-scale, 1)) !important;',
        '  opacity: 0 !important;',
        '  pointer-events: none !important;',
        '  transition: transform .28s cubic-bezier(.22,.61,.36,1), opacity .2s ease !important;',
        '}',
        'body.' + BODY_CLASS + ' .apple-native-leftdock.is-visible {',
        '  transform: translateX(0) translateY(-50%) !important;',
        '  opacity: 1 !important;',
        '  pointer-events: auto !important;',
        '}',
        'body.' + BODY_CLASS + ' .apple-native-leftdock__inner { display: flex; flex-direction: column; gap: .12em; }',
        'body.' + BODY_CLASS + ' .apple-native-leftdock__case { display: flex; flex-direction: column; gap: .04em; }',
        'body.' + BODY_CLASS + ' .apple-native-leftdock__split { height: 1px; margin: .3em .4em; background: rgba(255,255,255,.10); }',
        'body.' + BODY_CLASS + ' .apple-native-leftdock__item.selector {',
        ...
        '  cursor: pointer;',
        '}',
        'body.' + BODY_CLASS + ' .apple-native-leftdock__item.is-active,',
        'body.' + BODY_CLASS + ' .apple-native-leftdock__item.hover,',
        'body.' + BODY_CLASS + ' .apple-native-leftdock__item.focus {',
        '  background: rgba(255,255,255,.16) !important;',
        '  box-shadow: inset 0 1px 0 rgba(255,255,255,.14) !important;',
        '  transform: translateX(.04em) !important;',
        '}',
        'body.' + BODY_CLASS + ' .apple-native-leftdock__item .menu__ico { width: 1.5em; height: 1.5em; min-width: 1.5em; display: inline-flex; align-items: center; justify-content: center; flex: 0 0 auto; margin-right: .3em !important; color: currentColor; }',
        'body.' + BODY_CLASS + ' .apple-native-leftdock__item .menu__ico svg { width: 1.5em; height: 1.5em; color: currentColor; }',
        'body.' + BODY_CLASS + ' .apple-native-leftdock__item .menu__text { display: inline-block; flex: 1 1 auto; }',

        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .apple-native-leftdock {',
        '  backdrop-filter: none !important;',
        '  -webkit-backdrop-filter: none !important;',
        '  background: rgba(22,24,30,.94) !important;',
        '  box-shadow: 0 4px 12px rgba(0,0,0,.4) !important;',
        '}',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .apple-native-leftdock {',
        '  backdrop-filter: none !important;',
        '  -webkit-backdrop-filter: none !important;',
        ...
        '  transform: translateX(-130%) translateY(-50%) !important;',
        '}',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .apple-native-leftdock.is-visible {',
        '  transform: translateX(0) translateY(-50%) !important;',
        '}',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .apple-native-leftdock__item.selector,',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .apple-native-leftdock__item.selector {',
        '  transition: background .12s ease !important;',
        '}',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .apple-native-leftdock__item.selector {',
        '  transition: none !important;',
        '}',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .apple-native-leftdock__item.is-active,',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .apple-native-leftdock__item.hover,',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .apple-native-leftdock__item.focus,',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .apple-native-leftdock__item.is-active,',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .apple-native-leftdock__item.hover,',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .apple-native-leftdock__item.focus {',
        '  transform: none !important;',
        '  box-shadow: none !important;',
        '}',
        ...
        '  pointer-events: none !important;',
        '  font-size: calc(clamp(0.62rem, 0.34rem + 1.45vmin, 2.05rem) * var(--apple-native-scale, 1)) !important;',
        '}',
        ...
        '}',
        '@keyframes appleNativeSettingsLayerIn { from { opacity: 0; } to { opacity: 1; } }',
        'body.' + BODY_CLASS + '.settings--open .settings__layer { animation: appleNativeSettingsLayerIn .25s ease both; }',
        ...
        '  animation: appleNativeSelectboxLayerIn .25s ease both;',
        '}',
        '@keyframes appleNativeSelectboxLayerIn { from { opacity: 0; } to { opacity: 1; } }',
        ...
        '  animation: appleNativeSelectboxIn .28s cubic-bezier(.22,.61,.36,1) both;',
        '}',
        '@keyframes appleNativeSelectboxIn {',
        ...
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .selectbox-item.hover { transform: none !important; outline: 2px solid rgba(255,255,255,.22) !important; outline-offset: -2px !important; box-shadow: none !important; }',
        ...
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .settings-folder.hover { outline: 2px solid rgba(255,255,255,.22) !important; outline-offset: -2px !important; box-shadow: none !important; transform: none !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .selectbox-item__checkbox { transition: none !important; }',

        '@media (max-width: 767px) {',
        '  body.' + BODY_CLASS + ' .apple-native-topnav-shell { display: none !important; }',
        '  body.' + BODY_CLASS + ' .apple-native-topnav-rightdock { font-size: 1.3em !important; }',
    ...
    function sfSym(ch) {
      return '<span style="font-family:\'SF Symbols\',-apple-system,sans-serif;font-size:1em;font-weight:400;line-height:1;display:flex;align-items:center;justify-content:center;">' + ch + '</span>';
    }

    function iconSearch() { return sfSym('􀊫'); }
    function iconSettings() { return sfSym('􀍟'); }
    ...
    function iconFavorite() { return sfSym('􀉞'); }
    ...
    function iconEdit() { return sfSym('􀈎'); }
    ...
    function pickLeftdockIcon(action, label) {
      ...
      if (a === 'main') return sfSym('􁒖');
      if (a === 'movie' || a === 'cartoon') return sfSym('􁚱');
      if (a === 'tv' || a === 'anime') return sfSym('􀎳');
      ...
      if (a === 'torrent' || a === 'torrents') return sfSym('􀀬');
      if (a === 'console') return sfSym('􂝖');
      if (a === 'info' || a === 'notice') return sfSym('􀅵');
      if (a === 'history') return sfSym('􀐫');
    ...
    function getProfileButtonHtml() {
      try {
        ...
        if (icon && protocol && domain) {
          return '<img class="apple-native-topnav-right__profile-img" src="' + protocol + domain + '/img/profiles/' + icon + '.png" alt="profile">';
        }
      } catch (e) { }
      return iconProfile();
    }
    ...
    function bindControlPanelOutsideClose() {
      ...
      controlPanelOutsideHandler = function (e) {
        ...
        if (!target) return;
        if (target.closest && target.closest('.apple-native-control-panel')) return;
        if (target.closest && target.closest('.apple-native-topnav-clock')) return;
    ...
    function ensureControlPanelController(panel) {
      ...
      Lampa.Controller.add('apple_native_control_panel', {
        toggle: function () {
          var view = $(panel);
          var target = qs('.apple-native-control-panel__tile.selected', panel) || qs('.apple-native-control-panel__tile.selector', panel);
    ...
    function ensureControlPanel(head) {
      ...
      if (!head) return null;
      var panel = qs('.apple-native-control-panel', head);
      if (!panel) {
        panel = document.createElement('div');
        panel.className = 'apple-native-control-panel';
        panel.setAttribute('aria-hidden', 'true');
        head.appendChild(panel);
      }

      panel.innerHTML = '<div class="apple-native-control-panel__title">' + escapeHtml(langText('settings_cub_account', 'Account')) + '</div><div class="apple-native-control-panel__grid"></div>';
      var grid = qs('.apple-native-control-panel__grid', panel);
      ...
      [
        ...
        tile.className = 'apple-native-control-panel__tile selector';
        tile.setAttribute('data-role', def.role);
        tile.setAttribute('data-selector', 'true');
        tile.setAttribute('tabindex', '0');
        tile.innerHTML = '<span class="apple-native-control-panel__icon">' + def.icon + '</span><span class="apple-native-control-panel__label">' + escapeHtml(def.title) + '</span>';
    ...
    function openControlPanel(head) {
      ...
      try {
        ...
        if (window.Lampa && Lampa.Controller && typeof Lampa.Controller.toggle === 'function') {
          Lampa.Controller.toggle('apple_native_control_panel');
        }
      } catch (e) { }
      return true;
    }
    ...
    function closeControlPanel(restoreController) {
      var panel = qs('.apple-native-control-panel');
    ...
    function bindAction(btn, fn) {
      ...
      if (window.$) {
        try {
          $(btn).off('.appleNativeTopnavAction');
          $(btn).on('hover:enter.appleNativeTopnavAction', run);
          $(btn).on('hover:focus.appleNativeTopnavAction hover:hover.appleNativeTopnavAction', function () {
            btn.classList.add('focus');
          });
          $(btn).on('hover:blur.appleNativeTopnavAction hover:out.appleNativeTopnavAction', function () {
    ...
    function bindMenu(btn, actionName, sourceNode) {
      ...
      if (window.$) {
        try {
          $(btn).off('.appleNativeTopnavMenu');
          $(btn).on('hover:enter.appleNativeTopnavMenu', run);
          $(btn).on('hover:focus.appleNativeTopnavMenu hover:hover.appleNativeTopnavMenu', function () {
            btn.classList.add('focus');
          });
          $(btn).on('hover:blur.appleNativeTopnavMenu hover:out.appleNativeTopnavMenu', function () {
            btn.classList.remove('focus');
          });
        } catch (e) { }
      }
    ...
    function forwardWheelBelowTopnav(e) {
      if (e.__appleNativeForwardedWheel) return;
      var head = qs('.head');
      if (!head) return;
      ...
      try {
        ...
        clone.__appleNativeForwardedWheel = true;
    ...
    function attachTopnavWheelForwarding() {
      var head = qs('.head');
      if (!head || head.__appleNativeWheelBound) return;
      head.__appleNativeWheelBound = true;
    ...
    function neutralizeMenuController() {
      ...
      try {
        Lampa.Controller.add('menu', {
          toggle: function () {
            try { Lampa.Controller.toggle('apple_native_leftdock'); } catch (e) { }
          },
          update: function () { },
          left:  function () { try { Lampa.Controller.toggle('apple_native_leftdock'); } catch (e) { } },
          right: function () { try { Lampa.Controller.toggle('content'); }          catch (e) { } },
          up:    function () { try { Lampa.Controller.toggle('apple_native_leftdock'); } catch (e) { } },
      ...
      try {
        Lampa.Controller.add('head', {
          toggle: function () {
            ...
            if (!headEl) return;
            var view = $(headEl);
            var firstItem = qs('.apple-native-topnav-shell__item.selector', headEl)
              || qs('.apple-native-topnav-shell .selector', headEl)
              || qs('.selector', headEl);
            Lampa.Controller.collectionSet(view);
    ...
    function showLeftdock() {
      var dock = qs('.apple-native-leftdock');
      if (!dock) return;
    ...
    function hideLeftdock(immediate, force) {
      ...
      if (!force && window.Lampa && Lampa.Controller && typeof Lampa.Controller.enabled === 'function') {
        ...
        if (enabled && enabled.name === 'apple_native_leftdock') return;
      }
      if (immediate) {
        if (leftdockHoverHideTimer) {
          clearTimeout(leftdockHoverHideTimer);
    ...
    function watchSettingsLifecycle() {
      ...
      var attachInner = function () {
        ...
        if (!sb || sb.__appleNativeSettingsScrollObserver) return;
        ...
        sb.__appleNativeSettingsScrollObserver = inner;
      };

      var detachInner = function () {
        var sb = qs('.settings__body');
        if (sb && sb.__appleNativeSettingsScrollObserver) {
          sb.__appleNativeSettingsScrollObserver.disconnect();
          sb.__appleNativeSettingsScrollObserver = null;
        }
      };
    ...
    function patchControllerToggleForLeftdock() {
      ...
      try {
        ...
        Lampa.Controller.toggle = function (name) {
          if (name && name !== 'apple_native_leftdock') hideLeftdock(true, true);
    ...
    function bindHeadMenuIconClick() {
      ...
      if (!btn || btn.__appleNativeMenuIconBound) return;
      btn.__appleNativeMenuIconBound = true;
      ...
      var trigger = function (e) {
        ...
        try {
          ...
          if (enabled && enabled.name === 'apple_native_leftdock') {
            Lampa.Controller.toggle('content');
          } else {
            Lampa.Controller.toggle('apple_native_leftdock');
          }
        } catch (err) { }
      };
    ...
    function buildLeftdock() {
      ...
      if (!dock) {
        ...
        dock.className = 'apple-native-leftdock';
      ...
      inner.className = 'apple-native-leftdock__inner';
      ...
      Array.prototype.slice.call(sourceMenu.children).forEach(function (child) {
        ...
        if (child.classList.contains('menu__case')) {
          ...
          caseEl.className = 'apple-native-leftdock__case';
          qsa('.menu__item.selector', child).forEach(function (item) {
            if (item.classList.contains('hidden')) return;
            var btn = document.createElement('div');
            btn.className = 'apple-native-leftdock__item selector';
          ...
          split.className = 'apple-native-leftdock__split';
    ...
    function bindLeftdockAutoScroll(dock) {
      if (!dock || dock.__appleNativeAutoScrollObserver || typeof MutationObserver !== 'function') return;
      var observer = new MutationObserver(function (mutations) {
        for (var i = 0; i < mutations.length; i++) {
          ...
          if (!target.classList.contains('apple-native-leftdock__item')) continue;
      ...
      dock.__appleNativeAutoScrollObserver = observer;
    }

    function syncLeftdockActive() {
      var dock = qs('.apple-native-leftdock');
      if (!dock) return;
      qsa('.apple-native-leftdock__item[data-action]', dock).forEach(function (btn) {
    ...
    function registerLeftdockController(dock) {
      ...
      try {
        Lampa.Controller.add('apple_native_leftdock', {
          toggle: function () {
            var d = qs('.apple-native-leftdock');
            if (!d) return;
            showLeftdock();
            var view = $(d);
            var current = qs('.apple-native-leftdock__item.is-active', d)
              || qs('.apple-native-leftdock__item.selector', d);
            Lampa.Controller.collectionSet(view);
    ...
    function ensureRightDock(head) {
      if (!head) return null;
      var dock = qs('.apple-native-topnav-rightdock', head);
      if (!dock) {
        dock = document.createElement('div');
        dock.className = 'apple-native-topnav-rightdock';
    ...
    function ensureClock(head) {
      ...
      if (!clock) {
        ...
        clock.id = CLOCK_ID;
        clock.className = 'apple-native-topnav-clock selector';
    ...
    function ensureProfileButton(head) {
      if (!head) return null;
      var dock = ensureRightDock(head) || head;
      var profileBtn = qs('.apple-native-topnav-right__profile[data-role="profile"]', dock) || qs('.apple-native-topnav-right__profile[data-role="profile"]', head);
      if (!profileBtn) {
        profileBtn = document.createElement('div');
        profileBtn.className = 'apple-native-topnav-shell__item apple-native-topnav-right__profile selector';
    ...
    function patchTopnav() {
      ...
      var shell = qs('.apple-native-topnav-shell', head);
      if (!shell) {
        shell = document.createElement('div');
        shell.className = 'apple-native-topnav-shell';
        shell.innerHTML = '<div class="apple-native-topnav-shell__inner"><div class="apple-native-topnav-shell__items"></div><div class="apple-native-topnav-shell__right"></div></div>';
      }
      if (shell.parentNode !== head || head.firstChild !== shell) {
        head.insertBefore(shell, head.firstChild);
      }

      var itemsWrap = qs('.apple-native-topnav-shell__items', shell);
      var rightWrap = qs('.apple-native-topnav-shell__right', shell);
      ...
      getSelectedTopnavItems().forEach(function (def) {
        ...
        var btn = document.createElement('div');
        btn.className = 'apple-native-topnav-shell__item selector';
      ...
      iconItems.forEach(function (def) {
        var btn = document.createElement('div');
        btn.className = 'apple-native-topnav-shell__item apple-native-topnav-shell__item--icon selector';
      ...
      qsa('.apple-native-topnav-shell__item[data-action]', shell).forEach(function (btn) {
        btn.classList.remove('is-active');
        var source = getMenuItem(btn.getAttribute('data-action'));
        if (source && (source.classList.contains('active') || source.classList.contains('focus') || source.classList.contains('hover'))) {
          btn.classList.add('is-active');
    ...
    function getContinueEpisodeLabel(itemId) {
      try {
        var history = Lampa.Storage.get('history') || [];
        for (var i = 0; i < history.length; i++) {
          var h = history[i];
          if (!h) continue;
          var hId = h.movie ? h.movie.id : (h.id || 0);
          if (hId && itemId && String(hId) === String(itemId)) {
            var s = h.last_season, e = h.last_episode;
    ...
    function switchCardToBackdrop(cardEl) {
      ...
      var isContinue = !!cardEl.querySelector('.card-watched');
      if (isContinue) {
        cardEl.setAttribute('data-nfx-continue', '1');
        try {
          var row = cardEl.closest ? cardEl.closest('.items-line') : null;
          if (row && !row.hasAttribute('data-nfx-continue-row')) row.setAttribute('data-nfx-continue-row', '1');
        } catch (e) {}
      }
      ...
      if (cardImageMode === 'poster' && !isContinue) {
        if (imgEl) {
          if (imgEl.tagName === 'IMG') {
            if (!imgEl.hasAttribute('data-nfx-original-src')) {
              imgEl.setAttribute('data-nfx-original-src', imgEl.getAttribute('src') || '');
        ...
        function buildAppleTVPosterOverlay(logo) {
          if (!pView || pView.querySelector('.nfx-card-overlay')) return;
          var pOverlay = document.createElement('div');
          pOverlay.className = 'nfx-card-overlay';
          if (logo) {
            ...
            pOverlay.appendChild(img);
          }
          if (badgeEnabled() && (data.title || data.name)) {
            var pBadge = document.createElement('div');
            pBadge.className = 'nfx-card-logo';
            pBadge.textContent = data.name ? t('badge_tv') : t('badge_movie');
            pOverlay.appendChild(pBadge);
          }
          pView.appendChild(pOverlay);
          if (ratingEnabled() && pVote > 0) {
            var pRating = document.createElement('div');
        ...
        if (data.id && !isUltra) {
          fetchLogo(data.id, pTmdbType, function (logo) {
            buildAppleTVPosterOverlay(logo);
          });
        } else {
          buildAppleTVPosterOverlay(null);
        }
        return;
      }

      // Continue watching card in poster mode — load backdrop image
      if (isContinue && imgEl) {
        if (imgEl.tagName === 'IMG') {
          if (!imgEl.hasAttribute('data-nfx-original-src')) imgEl.setAttribute('data-nfx-original-src', imgEl.getAttribute('src') || '');
        } else if (!imgEl.hasAttribute('data-nfx-original-bg')) {
          imgEl.setAttribute('data-nfx-original-bg', imgEl.style.backgroundImage || '');
        }
        if (data.backdrop_path) setImgUrl(Lampa.TMDB.image('t/p/' + getBackdropQuality() + data.backdrop_path));
      }
      ...
      var metaHtml = genreNames.length ? '<div class="nfx-card-overlay__meta"><span>' + escapeHtml(genreNames.slice(0, 2).join(', ')) + '</span></div>' : '';
      ...
      if (!isContinue && badgeEnabled() && (data.title || data.name)) {
      ...
      if (isContinue && data.name && data.id) {
        var epLabel = getContinueEpisodeLabel(data.id);
        if (epLabel) {
          var epDiv = document.createElement('div');
          epDiv.className = 'nfx-continue-episode';
          epDiv.textContent = epLabel;
          overlay.appendChild(epDiv);
        }
      }
