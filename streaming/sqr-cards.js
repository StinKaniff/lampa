/**
 * SQR Cards (Cardify-style) — картка full start з трейлерами з TMDB.
 * Візуально та налаштуваннями повторює Cardify; трейлери беруться з TMDB API.
 * Підключає YouTube IFrame API самостійно.
 */
(function () {
    'use strict';

    if (typeof Lampa === 'undefined') {
        console.warn('sqr-cards.js: Lampa not found');
        return;
    }
    ...
    function State(object) {
        ...
        this.dispatch = function (actionName) {
            var action = object.transitions[actionName];
            if (action) action.call(this, this);
            else console.warn('sqr-cards: invalid action', actionName);
        };
        ...
        this.state = new State({
            ...
            transitions: {
                start: function (state) {
                    ...
                    if (self.player.display) state.dispatch('play');
                    else if (self.player.loaded) {
                        self.animate();
                        self.timer_load = setTimeout(function () { state.dispatch('load'); }, self.timelaunch);
                    }
                },
                load: function (state) {
                ...
                toggle: function (state) {
                    clearTimeout(self.timer_load);
                    if (Lampa.Controller.enabled().name === 'sqr_cards_trailer') return;
                    if (Lampa.Controller.enabled().name === 'full_start' && self.same()) state.start();
                    else if (self.player.display) state.dispatch('hide');
                },
                hide: function () {
                    self.player.pause();
                    self.player.hide();
    ...
    Trailer.prototype.animate = function () {
        var self = this;
        ...
        this.timer_anim = setInterval(function () {
            ...
            if (left > self.timelaunch) clearInterval(self.timer_anim);
            loader.width(Math.round((left / self.timelaunch) * 100) + '%');
        }, 100);
    };
    Trailer.prototype.preview = function () {
        var preview = $(
            '<div class="cardify-preview">' +
            '<div><img class="cardify-preview__img" />' +
            '<div class="cardify-preview__line one"></div>' +
            '<div class="cardify-preview__line two"></div>' +
            '<div class="cardify-preview__loader"></div></div></div>'
        );
        var self = this;
        Lampa.Utils.imgLoad(preview.find('img'), this.video.img, function () {
            preview.find('img').addClass('loaded');
        });
    ...
    Trailer.prototype.controll = function () {
        var self = this;
        function out() {
            self.state.dispatch('hide');
            Lampa.Controller.toggle('full_start');
        }
        Lampa.Controller.add('sqr_cards_trailer', {
        ...
        Lampa.Controller.toggle('sqr_cards_trailer');
    };
    Trailer.prototype.start = function () {
        var self = this;
        function toggle() { self.state.dispatch('toggle'); }
        function destroy(e) {
    ...
    function fetchVideosFromTMDB(card, callback) {
        ...
        var lang = (Lampa.Storage.get && Lampa.Storage.get('tmdb_lang')) || (Lampa.Storage.field && Lampa.Storage.field('tmdb_lang')) || (Lampa.Storage.get && Lampa.Storage.get('language')) || 'uk';
        var url = type + '/' + card.id + '/videos?api_key=' + Lampa.TMDB.key() + '&language=' + (lang || 'uk');
        var apiUrl = Lampa.TMDB.api(url);
        fetch(apiUrl)
    ...
    // —— Застосування картки (фон, деталі, статус, рейтинг, original poster) ——
    function applyCardifyLayout(e) {
        ...
        var storageField = (storage.field && typeof storage.field === 'function') ? storage.field : function (key) { return storage.get ? storage.get(key) : undefined; };
        if (!storageField.call(storage, 'cardify_show_status')) render.find('.full-start__status').hide();
        if (!storageField.call(storage, 'cardify_show_rating')) render.find('.full-start-new__rate-line.rate-fix').hide();
    ...
    function loadOriginalPoster(e, render) {
        ...
        if (e.data && e.data.movie && e.data.movie.backdrop_path) backdropPath = e.data.movie.backdrop_path;
        else if (e.data && e.data.backdrop_path) backdropPath = e.data.backdrop_path;
        else if (e.object && e.object.card && e.object.card.backdrop_path) backdropPath = e.object.card.backdrop_path;
        ...
        if (backdropPath && bgImg.length) {
            var originalUrl = 'https://image.tmdb.org/t/p/original' + (backdropPath.indexOf('/') === 0 ? backdropPath : '/' + backdropPath);
            var tempImg = new Image();
            tempImg.onload = function () { bgImg.attr('src', originalUrl); };
            tempImg.src = originalUrl;
        }
    }

    function startPlugin() {
        if (!Lampa.Platform || !Lampa.Platform.screen) {
            // Якщо немає TV-перевірки — працюємо на всіх платформах
        ...
        Lampa.Lang.add({
            ...
            cardify_enable_trailer: { ru: 'Показывать трейлер', en: 'Show trailer', uk: 'Показувати трейлер', be: 'Паказваць трэйлер', zh: '显示预告片', pt: 'Mostrar trailer', bg: 'Показване на трейлър' }
        });

        var fullStartNewHtml = '<div class="full-start-new cardify">' +
            '<div class="full-start-new__body">' +
            '<div class="full-start-new__left hide">' +
            '<div class="full-start-new__poster"><img class="full-start-new__img full--poster" /></div></div>' +
            ...
            '</svg><span>#{title_reactions}</span></div>' +
            ...
            '</svg><span>#{title_subscribe}</span></div>' +
            '<div class="full-start__button selector button--options">' +
            '<svg width="38" height="10" viewBox="0 0 38 10" fill="none" xmlns="http://www.w3.org/2000/svg">' +
            '<circle cx="4.88968" cy="4.98563" r="4.75394" fill="currentColor"/><circle cx="18.9746" cy="4.98563" r="4.75394" fill="currentColor"/><circle cx="33.0596" cy="4.98563" r="4.75394" fill="currentColor"/>' +
            '</svg></div></div></div>' +
            '<div class="cardify__right">' +
            '<div class="full-start-new__reactions selector"><div>#{reactions_none}</div></div>' +
            '<div class="full-start-new__rate-line">' +
            '<div class="full-start__pg hide"></div><div class="full-start__status hide"></div>' +
            '</div></div></div></div>' +
            '<div class="hide buttons--container">' +
            '<div class="full-start__button view--torrent hide"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="50px" height="50px"><path d="M25,2C12.317,2,2,12.317,2,25s10.317,23,23,23s23-10.317,23-23S37.683,2,25,2z M40.5,30.963c-3.1,0-4.9-2.4-4.9-2.4 S34.1,35,27,35c-1.4,0-3.6-0.837-3.6-0.837l4.17,9.643C26.727,43.92,25.874,44,25,44c-2.157,0-4.222-0.377-6.155-1.039L9.237,16.851 c0,0-0.7-1.2,0.4-1.5c1.1-0.3,5.4-1.2,5.4-1.2s1.475-0.494,1.8,0.5c0.5,1.3,4.063,11.112,4.063,11.112S22.6,29,27.4,29 c4.7,0,5.9-3.437,5.7-3.937c-1.2-3-4.993-11.862-4.993-11.862s-0.6-1.1,0.8-1.4c1.4-0.3,3.8-0.7,3.8-0.7s1.105-0.163,1.6,0.8 c0.738,1.437,5.193,11.262,5.193,11.262s1.1,2.9,3.3,2.9c0.464,0,0.834-0.046,1.152-0.104c-0.082,1.635-0.348,3.221-0.817,4.722 C42.541,30.867,41.756,30.963,40.5,30.963z" fill="currentColor"/></svg><span>#{full_torrents}</span></div>' +
        ...
        var icon = '<svg width="36" height="28" viewBox="0 0 36 28" fill="none" xmlns="http://www.w3.org/2000/svg">' +
            ...
            '<rect x="25" y="20" width="6" height="3" rx="1.5" fill="white"/></svg>';
